...
module.exports = {
	connect: function(jsfs_server){
		
		// private variables and methods
		var _jsfs_server = jsfs_server;
		var _keychain_root = jsfs_server + "/keychains"; //"http://localhost:7302/keychains";
		var _keychain = {};
		_keychain.keys = {};
		...
		return {
			...
			//connect: function(s){
			//	_jsfs_server = s;
			//	_keychain_root = _jsfs_server + "/keychains";
			//},
			...
			load_object: function(url, result_callback){
				...
				// debug
				console.log(_jsfs_server);
				
				xhr.open("GET", _jsfs_server + url, true);
				//xhr.setRequestHeader("x-access-token", _hashed_token);
				xhr.onreadystatechange = parse_response;
				xhr.send();
